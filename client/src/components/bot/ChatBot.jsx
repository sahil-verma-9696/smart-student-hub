...
const Chatbot = () => {
  ...
  const API_ROUTES = {
    skill: '/mind-piolet/skill',
    roadmap: '/mind-piolet/roadmap',
    interview: '/mind-piolet/interview',
    'gap-finder': '/mind-piolet/skill',
    guidance: '/mind-piolet/skill',
  };
  ...
  const callBackendAPI = async (userMessage) => {
    setLoading(true);
    try {
      const endpoint = API_ROUTES[selectedFeature] || '/mind-piolet/skill';
      // Try getting token from multiple possible keys
      const token = localStorage.getItem('token') || localStorage.getItem('access-token');

      if (!token) {
        setMessages((prev) => [
          ...prev,
          { role: 'assistant', content: '⚠ Please log in first to use Mind Pilot features.' },
        ]);
        setLoading(false);
        return;
      }
      ...
      // Handle nested response structure (data.data.result) and error objects
      const content = data?.data?.result || data?.result || data?.error?.message || data?.message || '⚠ No response from backend.';
      ...
      setMessages((prev) => [
        ...prev,
        { role: 'assistant', content: '❌ Backend Error! Please check the server.' },
      ]);
    }
    setLoading(false);
  };
        ...
        {features.map((feature) => (
          <button
            ...
            style={{
              padding: '10px 14px',
              backgroundColor:
                selectedFeature === feature.id ? COLORS.indigo : COLORS.card,
              color:
                selectedFeature === feature.id ? 'white' : COLORS.textDark,
              borderRadius: '12px',
          ...
          >
            <div
              style={{
                ...
                borderRadius: '16px',
                backgroundColor:
                  msg.role === 'user' ? COLORS.primary : COLORS.card,
