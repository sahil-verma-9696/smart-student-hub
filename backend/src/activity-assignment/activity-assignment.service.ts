...
import { Activity } from '../activity/schema/acivity.schema';
...
export class ActivityAssignmentService {
	...
	async autoAssignFaculty(activityId: string, instituteId: string): Promise<ActivityAssignment> {
		...
		const academic = await this.academicModel.findById(student.academicDetails).exec();
		...
		if (studentBranchId) {
			candidateFaculty = allFaculty.filter((f) => 
				f.teachesBranches && 
				f.teachesBranches.some((b) => b.toString() === studentBranchId.toString())
			);
		}
		...
		for (const assignment of assignments) {
			...
			if (activity) {
				if (activity.status === 'PENDING') {
					pendingReviews++;
				} else if (activity.status === 'APPROVED' || activity.status === 'REJECTED') {
					completedReviews++;
				}
			}
		}
	 ...
	 */
	async getUnassignedActivities(instituteId?: string): Promise<ActivityAssignment[]> {
	 ...
	 */
	async bulkAssignFaculty(activityIds: string[], facultyId: string): Promise<number> {
		if (!Types.ObjectId.isValid(facultyId)) {
			throw new BadRequestException('Invalid faculty ID');
		}
