...
export class ActivityTypeService {
	...
	async create(dto: CreateActivityTypeDto, user: JwtPayload): Promise<ActivityType> {
		...
		const activityTypeData: any = {
			...
			status,
			createdBy: new Types.ObjectId(user.userId), // Track who created this type
		};
	...
	async approveActivityType(id: string, user: JwtPayload): Promise<ActivityType> {
		...
		if (type.status === 'APPROVED') {
			throw new BadRequestException('Activity type is already approved');
		}
		...
		type.approvedBy = new Types.ObjectId(user.userId);
	...
	async rejectActivityType(id: string, user: JwtPayload): Promise<ActivityType> {
		...
		if (type.status === 'REJECTED') {
			throw new BadRequestException('Activity type is already rejected');
		}
		...
		// Update status and rejector
		type.status = 'REJECTED';
		type.rejectedBy = new Types.ObjectId(user.userId);
		type.updatedAt = new Date();
		await type.save();
